...
$config{'ThanksPage'} = 'http://s-zaiko.com/kouji/thanks_kameiten/';

## 設置者に届くメールの件名
$config{'subject'} = '[ %s ] ◆◆◆ 加盟店登録◆◆◆';

## 設置者に届くメールの本文整形
$_TEXT{'posted'} = <<'__posted_body__';

<_mfp_date_>
加盟店登録フォームより以下のメールを受付ました。
──────────────────────────
受付番号：<_mfp_serial_>
...
$config{"ReturnSubject"} = '[ %s ] 加盟店登録のお申し込みありがとうございました';
